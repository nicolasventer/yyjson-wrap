...
DOCTEST_DEFINE_DECORATOR(only, bool, true);
DOCTEST_DEFINE_DECORATOR(ignore_only, bool, true);
...
namespace detail {
    ...
    TestCase::TestCase(funcType test, const char* file, unsigned line, const TestSuite& test_suite,
        ...
        m_only              = test_suite.m_only;
        m_ignore_only       = test_suite.m_ignore_only;
...
int Context::run() {
    ...
    // check if at least one test has only attribute
    ...
    for(auto& curr : testArray) {
        ...
        if(tc.m_skip && !p->no_skip)
            skip_me = true;
        if(!tc.m_ignore_only && only_present && !tc.m_only && !p->no_skip)
            skip_me = true;
